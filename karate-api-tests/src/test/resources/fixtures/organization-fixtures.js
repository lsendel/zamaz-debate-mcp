...
function fn() {
    ...
    const orgTestData = karate.callSingle('classpath:test-data/organizations.json');
    ...
    const organizationFixtures = {
        ...
        createOrganization: function(orgData, authToken) {
            ...
            const finalOrgData = Object.assign(defaultOrgData, orgData);

            // If no auth token provided, use admin token
            if (!authToken) {
                let adminAuth = authFixtures.login(config.testData.adminUser.email, config.testData.adminUser.password);
                authToken = adminAuth.token;
            }

            const createResponse = karate.call('classpath:organization/create-organization.feature', {
            ...
            if (createResponse.response && createResponse.response.id) {
                let org = createResponse.response;
        ...
        getOrganization: function(orgId, authToken) {
            ...
            const getResponse = karate.call('classpath:organization/get-organization.feature', {
        ...
        updateOrganization: function(orgId, updateData, authToken) {
            ...
            const updateResponse = karate.call('classpath:organization/update-organization.feature', {
                organizationId: orgId,
            ...
            if (updateResponse.response && updateResponse.response.id) {
                let org = updateResponse.response;
                organizationFixtures.organizationCache[org.id] = org;
                return org;
            }
        ...
        deleteOrganization: function(orgId, authToken) {
            ...
            const deleteResponse = karate.call('classpath:organization/delete-organization.feature', {
                organizationId: orgId,
                authToken: authToken,
                baseUrl: config.serviceUrls.organization
            });
        ...
        listOrganizations: function(filters, authToken) {
            ...
            const listResponse = karate.call('classpath:organization/list-organizations.feature', {
                filters: filters || {},
                authToken: authToken,
                baseUrl: config.serviceUrls.organization
            });
        ...
        addUserToOrganization: function(orgId, userData, authToken) {
            ...
            const addUserResponse = karate.call('classpath:organization/add-user.feature', {
                organizationId: orgId,
        ...
        removeUserFromOrganization: function(orgId, userId, authToken) {
            ...
            const removeUserResponse = karate.call('classpath:organization/remove-user.feature', {
                organizationId: orgId,
                userId: userId,
                authToken: authToken,
                baseUrl: config.serviceUrls.organization
        ...
        listOrganizationUsers: function(orgId, authToken) {
            ...
            const listUsersResponse = karate.call('classpath:organization/list-users.feature', {
                organizationId: orgId,
                authToken: authToken,
                baseUrl: config.serviceUrls.organization
            });
        ...
        updateUserRole: function(orgId, userId, newRole, authToken) {
            ...
            const updateRoleResponse = karate.call('classpath:organization/update-user-role.feature', {
                organizationId: orgId,
                userId: userId,
        ...
        createOrganizationWithUsers: function(orgData, userCount) {
            ...
            // Create organization
            const org = organizationFixtures.createOrganization(orgData);

            // Create admin user
            const adminUser = organizationFixtures.addUserToOrganization(org.id, {
            ...
            for (var i = 0; i < userCount; i++) {
                const user = organizationFixtures.addUserToOrganization(org.id, {
        ...
        getOrganizationStats: function(orgId, authToken) {
            if (!authToken) {
                const adminAuth = authFixtures.login(config.testData.adminUser.email, config.testData.adminUser.password);
                authToken = adminAuth.token;
            }

            const statsResponse = karate.call('classpath:organization/get-organization-stats.feature', {
                organizationId: orgId,
                authToken: authToken,
                baseUrl: config.serviceUrls.organization
            });
        ...
        createOrganizationForTier: function(tier, authToken) {
            const orgData = organizationFixtures.getTestOrganizationData('default');

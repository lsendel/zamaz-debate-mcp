...
class GitHubAPISimulator:
    ...
    def __init__(self, config: RateLimitConfig):
        ...
        # Rate limit tracking
        self.rate_limits = {
            RateLimitType.CORE: {'remaining': config.core_limit, 'reset_time': time.time() + config.core_window_seconds},
            RateLimitType.SEARCH: {'remaining': config.search_limit, 'reset_time': time.time() + config.search_window_seconds},
            RateLimitType.GRAPHQL: {'remaining': config.graphql_limit, 'reset_time': time.time() + config.graphql_window_seconds}
        }
    ...
    async def simulate_api_call(self, endpoint: str, method: str = 'GET', 
    ...
    async def _check_abuse_detection(self):
        ...
        self.request_history = [
            ...
            if current_time - req['timestamp'] < self.config.abuse_window_seconds
        ]
    ...
    def _reset_rate_limit(self, rate_limit_type: RateLimitType):
        ...
        self.rate_limits[rate_limit_type] = {
            'remaining': limit,
    ...
    def _generate_response(self, endpoint: str, method: str, payload: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        if '/user' in endpoint:
            ...
            return {
                'total_count': random.randint(1, 1000),
...
class GitHubAPIRateLimitTest(PerformanceTestBase):
    ...
    async def _run_scenario(self, scenario: Dict[str, Any]):
        ...
        while time.time() < end_time:
            ...
            endpoint, method, rate_limit_type = random.choice(endpoints)
            
            # Make API call
            result = await self.api_simulator.simulate_api_call(
                endpoint, method, rate_limit_type
            )
        ...
        self.metrics.record_operation(
            f"scenario_summary_{scenario['name']}",
    ...
    async def test_concurrent_rate_limiting(self):
        ...
        for worker_id in range(10):
            worker = asyncio.create_task(
                concurrent_requester(worker_id, 50)
            )

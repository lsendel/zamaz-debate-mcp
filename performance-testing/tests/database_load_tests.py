...
class DatabaseSchema:
    ...
    def _create_tables(self) -> dict[str, Table]:
        ...
        return {"users": users, "posts": posts, "comments": comments, "activity_log": activity_log}
    ...
    def create_indexes(self):
        ...
        with self.engine.connect() as conn:
            ...
            conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS idx_activity_resource ON load_test_activity_log(resource_type, resource_id)"
                )
            )

            # Composite indexes for common queries
            conn.execute(
                text("CREATE INDEX IF NOT EXISTS idx_posts_user_published ON load_test_posts(user_id, is_published)")
            )
            conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS idx_comments_post_approved ON load_test_comments(post_id, is_approved)"
                )
            )
...
class DatabaseLoadTest(PerformanceTestBase):
    ...
    def __init__(self, config: TestConfiguration, db_config: DatabaseLoadConfig):
        ...
        self.schema = None
        self.test_data = {"users": [], "posts": [], "comments": []}
    ...
    async def _generate_initial_data(self):
            ...
            posts_data.append(
                {
                    ...
                    "view_count": secrets.randbelow(0, 1000),
                    "like_count": secrets.randbelow(0, 100),
                }
            )
        ...
        for i in range(min(self.db_config.initial_data_size, 100000)):
            post_id, post_user_id = secrets.choice(post_data)
            comments_data.append(
                {"post_id": post_id, "user_id": secrets.choice(user_ids), "content": self._generate_random_comment()}
            )
    ...
    def _generate_random_content(self) -> str:
        ...
        for _ in range(secrets.randbelow(3, 8)):
            content.append(secrets.choice(sentences))
            ...
            for _ in range(self.db_config.transactions_per_user):
                try:
                    async with self.connection_pool.acquire() as conn, conn.transaction():
                        ...
                        user_id = secrets.choice(self.test_data["users"])
                        ...
                        if self.test_data["posts"]:
                            post_id = secrets.choice(self.test_data["posts"])[0]
                            await conn.execute(
                                """
    ...
    async def _test_query_performance(self):
        ...
        query_types = [
            {
                ...
                "params": lambda: [secrets.choice(self.test_data["users"])],
            },
            {
                "name": "complex_join",
        ...
        async def query_worker(worker_id: int):
            ...
            for _ in range(100):
                ...
                query_type = secrets.choices(query_types, weights=[qt["weight"] for qt in query_types])[0]
                ...
                try:
                    async with self.connection_pool.acquire() as conn:
                        ...
                        self.metrics.record_operation(
                            ...
                            duration,
                            True,
                            {"worker_id": worker_id, "rows_returned": len(result)},
                        )
        ...
        for query_type in query_types:
            ...
            total_count = sum(r["query_results"][query_name]["count"] for r in successful_workers)
            total_time = sum(r["query_results"][query_name]["total_time"] for r in successful_workers)
        ...
        self.metrics.record_operation(
            "query_performance", duration, True, {"workers": len(workers), "query_stats": query_stats}
        )
            ...
            for _ in range(200):
                try:
                    async with self.connection_pool.acquire() as conn:
                        ...
                        operation = secrets.choice(["user_posts", "post_comments", "user_activity", "popular_posts"])
            ...
            for _ in range(50):
                try:
                    async with self.connection_pool.acquire() as conn:
                        ...
                        operation = secrets.choice(["create_post", "create_comment", "update_views", "log_activity"])
    ...
    async def _test_deadlock_handling(self):
        ...
        deadlock_rate = (
            total_deadlocks / (total_operations + total_deadlocks) if (total_operations + total_deadlocks) > 0 else 0
        )
    ...
    async def _test_index_performance(self):
        ...
        index_queries = [
            {
                ...
                "params": lambda: [f"user_{secrets.randbelow(0, 9999):06d}"],
            },
            {
                "name": "posts_by_user",
            ...
            },
            {
                ...
                "params": lambda: [secrets.choice(self.test_data["posts"])[0]] if self.test_data["posts"] else [1],
            },
            {
                ...
                for _ in range(20):
                    try:
                        async with self.connection_pool.acquire() as conn:
                            ...
                            result = await conn.fetch(query_config["query"], *query_config["params"]())

                            duration = (time.time() - start_time) * 1000
                            query_results[query_name].append({"duration": duration, "rows": len(result)})
        ...
        self.metrics.record_operation(
            "index_performance", duration, True, {"workers": len(workers), "index_stats": index_stats}
        )
        ...
        async def normal_operation_worker(worker_id: int):
            ...
            for _ in range(100):
                try:
                    async with self.connection_pool.acquire() as conn:
                        # Random operation
                        operation = secrets.choice(["read", "write"])
...
# Export main classes
__all__ = ["DatabaseLoadConfig", "DatabaseLoadTest", "DatabaseSchema"]

...
class DockerTestConfig:
    ...
    # Test environment
    test_services: List[Dict[str, Any]] = field(default_factory=lambda: [
        {
            'name': 'postgres',
            'image': 'postgres:15',
            'environment': {
...
class DockerPerformanceTestRunner:
    ...
    async def _create_test_container(self, test_module: str, test_config: Dict[str, Any]) -> Container:
        ...
        try:
            ...
            artifacts_volume = self.docker_client.volumes.create(
                f"test-artifacts-{int(time.time())}"
            )
            ...
            container = self.docker_client.containers.create(
                ...
                network=self.config.network_name,
                volumes={
                    artifacts_volume.name: {'bind': '/artifacts', 'mode': 'rw'}
                },
                environment={
...
"""
        ...
        if monitoring_data:
            ...
            ) / len(monitoring_data) if monitoring_data else 0

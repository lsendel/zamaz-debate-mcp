    ...
    def __init__(self, config: TestConfiguration):
        ...
        if config.enable_prometheus_metrics:
            ...
            self.request_counter = Counter(
                "test_requests_total", "Total test requests", ["method", "endpoint", "status"], registry=self.registry
            )
            ...
            self.memory_gauge = Gauge("test_memory_usage_mb", "Memory usage in MB", registry=self.registry)
            self.cpu_gauge = Gauge("test_cpu_usage_percent", "CPU usage percentage", registry=self.registry)
    ...
    def stop_monitoring(self):
        ...
        if self.operations:
            ...
            if self.metrics.duration_ms > 0:
                self.metrics.operations_per_second = self.metrics.operations_count / (self.metrics.duration_ms / 1000)

            success_count = sum(1 for op in self.operations if op.get("success", False))
            self.metrics.success_rate = (
                success_count / self.metrics.operations_count if self.metrics.operations_count > 0 else 0
            )
            self.metrics.error_count = len(self.error_log)

    def record_operation(self, name: str, duration_ms: float, success: bool = True, metadata: dict | None = None):
        ...
        with self.lock:
            ...
            if self.config.enable_prometheus_metrics:
                ...
                self.request_counter.labels(
                    method=metadata.get("method", "unknown"), endpoint=name, status=status
                ).inc()

                self.request_duration.labels(method=metadata.get("method", "unknown"), endpoint=name).observe(
                    duration_ms / 1000
                )
    ...
    def _get_error_summary(self) -> dict[str, Any]:
        ...
        for error in self.error_log:
            ...
            if len(error_types[error_type]["examples"]) < 3:
                error_types[error_type]["examples"].append({"error": error["error"], "context": error["context"]})

        return {"total_errors": len(self.error_log), "error_types": error_types}
...
class LoadTestRunner:
    ...
    async def run_constant_load(self, test_func: Callable, duration_seconds: int | None = None) -> list[dict[str, Any]]:
    ...
    async def run_burst_load(self, test_func: Callable, burst_size: int | None = None) -> list[dict[str, Any]]:
        ...
        for result in results:
            if isinstance(result, Exception):
                processed_results.append(
                    {"timestamp": time.time(), "duration_ms": 0, "success": False, "error": str(result), "result": None}
                )
            else:
    ...
    async def run_ramp_load(
        self, test_func: Callable, max_rps: int | None = None, ramp_duration: int | None = None
    ) -> list[dict[str, Any]]:

  ...
  try {
    ...
    const usernameSelector = await page.evaluateHandle(() => {
      ...
      for (let input of inputs) {
        if (input.type === 'text' || input.name === 'username' || input.id === 'username' || ;
            input./* TODO: Refactor nested ternary - placeholder?.toLowerCase().includes('username')) {
          return input;
        }
      }
      return null;
    ...
    const loginButton = await page.evaluateHandle(() => {
      ...
      for (let button of buttons) {
        if (button./* TODO: Refactor nested ternary - textContent?.toLowerCase().includes('login') ||;

...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("kiro_pr_processor.log")],
)
...
class PullRequestProcessor:
    ...
    def process(self):
        ...
        try:
            ...
            logger.error(f"Error processing PR #{self.pr_number}: {e!s}")
            return False
    ...
    def extract_pr_metadata(self):
        ...
        issue_refs = re.findall(
            r"(?:close[sd]?|fix(?:e[sd])?|resolve[sd]?)\s+(?:#(\d+)|([a-zA-Z0-9_-]+/[a-zA-Z0-9_-]+)#(\d+))",
            self.description,
            re.IGNORECASE,
        )
        ...
        for ref in issue_refs:
            if ref[0]:  # Format: #123
                self.linked_issues.append({"repo": self.repo_full_name, "number": ref[0]})
            elif ref[1] and ref[2]:  # Format: owner/repo#123
                self.linked_issues.append({"repo": ref[1], "number": ref[2]})
            ...
            # Process content lines
                ...
                if line.startswith("+"):
                    ...
                    changes_by_file[current_file].append(
                        {"type": "add", "line_number": current_new_line, "content": line[1:]}
                    )
                    current_new_line += 1
                elif line.startswith("-"):
                    # Removed line
                    changes_by_file[current_file].append(
                        {"type": "remove", "line_number": current_old_line, "content": line[1:]}
                    )
                    current_old_line += 1
    ...
    def gather_context_from_linked_issues(self):
        ...
        for issue in self.linked_issues:
            try:
                ...
                logger.warning(f"Error gathering context from issue #{issue['number']}: {e!s}")
    ...
    def analyze_code_changes(self):
        ...
        analysis_request = {
            ...
            "repository": {"full_name": self.repo_full_name, "owner": self.repo_owner, "name": self.repo_name},
            "files": file_data,
        ...
        try:
            ...
            self.issues = analyze_pr(
                pr_data={"number": self.pr_number, "repository": {"full_name": self.repo_full_name}},
                files=file_data,
                diff=self.diff,
                config=self.config,
            )
            ...
            try:
                ...
                suggestions = generate_fix_suggestions(
                    self.github, self.repo_full_name, self.pr_number, self.issues, file_data, self.config
                )
                ...
                for suggestion in suggestions:
                    ...
                    for i, existing_issue in enumerate(self.issues):
                        if (
                            existing_issue.get("file_path") == issue.get("file_path")
                            and existing_issue.get("line_start") == issue.get("line_start")
                            and existing_issue.get("message") == issue.get("message")
                        ):
                ...
                logger.error(f"Error generating fix suggestions: {e!s}")

        except Exception as e:
            logger.error(f"Error analyzing code changes: {e!s}")
            self.issues = []
    ...
    def generate_review_comments(self):
        ...
        try:
            ...
            logger.error(f"Error generating review comments: {e!s}")
    ...
    while is_running:
        try:
            ...
            processor = PullRequestProcessor(
                item["github_client"], item["repo_full_name"], item["pr_number"], item["config"]
            )
            ...
            logger.error(f"Error in queue processing thread: {e!s}")

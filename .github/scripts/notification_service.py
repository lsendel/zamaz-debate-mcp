...
class NotificationService:
        ...
        for channel in channels:
            try:
                if channel == NotificationChannel.SLACK:
                    results[channel.value] = await self._send_slack_notification(recipient, content)
                elif channel == NotificationChannel.EMAIL:
                    results[channel.value] = await self._send_email_notification(recipient, content)
                elif channel == NotificationChannel.GITHUB:
                    results[channel.value] = await self._send_github_notification(recipient, content)
                elif channel == NotificationChannel.WEBHOOK:
                    results[channel.value] = await self._send_webhook_notification(recipient, content)
            except Exception as e:
                logger.error(f"Error sending {channel.value} notification: {e!s}")
    ...
    async def _send_slack_notification(self, recipient: NotificationRecipient, content: NotificationContent) -> bool:
        ...
        try:
            ...
            logger.error(f"Slack notification error: {e!s}")
            return False

    def _format_slack_message(self, recipient: NotificationRecipient, content: NotificationContent) -> dict[str, Any]:
        """Format notification content for Slack."""
        # Color based on priority
        color_map = {"critical": "#dc3545", "high": "#fd7e14", "normal": "#0366d6", "low": "#28a745"}
        ...
        message = {"blocks": blocks, "attachments": attachments}
    ...
    async def _send_email_notification(self, recipient: NotificationRecipient, content: NotificationContent) -> bool:
        ...
        try:
            ...
            logger.error(f"Email notification error: {e!s}")
            return False

    def _format_email_html(self, recipient: NotificationRecipient, content: NotificationContent) -> str:
    ...
    def _format_email_text(self, recipient: NotificationRecipient, content: NotificationContent) -> str:
...
"""
    ...
    async def _send_github_notification(self, _recipient: NotificationRecipient, content: NotificationContent) -> bool:
        ...
        try:
            ...
            if content.pull_request:
                ...
                async with (
                    aiohttp.ClientSession() as session,
                    session.post(url, headers=headers, json={"body": comment_body}) as response,
                ):
            ...
            logger.error(f"GitHub notification error: {e!s}")
            return False
    ...
    async def _send_webhook_notification(self, recipient: NotificationRecipient, content: NotificationContent) -> bool:
        ...
        try:
            payload = {
                ...
                "recipient": {"user_id": recipient.user_id, "username": recipient.username},
                "timestamp": datetime.now().isoformat(),
            }

            async with (
                aiohttp.ClientSession() as session,
                session.post(webhook_url, json=payload, headers={"Content-Type": "application/json"}) as response,
            ):
                return response.status in [200, 201, 202, 204]

        except Exception as e:
            logger.error(f"Webhook notification error: {e!s}")
            return False

    def _get_preferred_channels(
        self, recipient: NotificationRecipient, notification_type: NotificationType
        ...
        type_prefs = preferences.get("notification_types", {}).get(notification_type.value, {})
    ...
    async def queue_notification(self, recipient: NotificationRecipient, content: NotificationContent):
        """Queue a notification for batch processing."""
        await self.notification_queue.put({"recipient": recipient, "content": content, "timestamp": datetime.now()})
        ...
        while True:
            ...
            while datetime.now() < deadline:
                try:
                    ...
                    notification = await asyncio.wait_for(self.notification_queue.get(), timeout=max(timeout, 0.1))
    ...
    async def _process_notification_batch(self, notifications: list[dict[str, Any]]):
        ...
        for (recipient_id, notif_type), group in grouped.items():
            if len(group) == 1:
                ...
                await self.send_notification(group[0]["recipient"], group[0]["content"])
            else:
                # Create and send digest
                digest_content = self._create_digest_notification(notif_type, group)
                await self.send_notification(group[0]["recipient"], digest_content)

    def _create_digest_notification(
        self, notification_type: NotificationType, notifications: list[dict[str, Any]]
    ) -> NotificationContent:
    ...
    async def _log_notification_attempt(
        self, recipient: NotificationRecipient, content: NotificationContent, results: dict[str, bool]
    ):
    ...
    async def send_weekly_summary(
        self, recipient: NotificationRecipient, summary_data: dict[str, Any]
    ) -> dict[str, bool]:
...
if __name__ == "__main__":
    ...
    # Example recipient
    recipient = NotificationRecipient(
        user_id="user123", username="johndoe", email="john@example.com", slack_id="U12345678"
    )
    ...
    async def test():
        await notify_review_completed(
            ...
            "myorg/myrepo",
            42,
            {"total_issues": 5, "critical_issues": 0, "suggestions_made": 12, "estimated_time_saved": "30 minutes"},
        )

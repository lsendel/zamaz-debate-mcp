...
class ComprehensiveAnalysisSystem:
    ...
    def __init__(self, project_path: str, github_token: str | None = None):
    ...
    def run_comprehensive_analysis(self) -> dict[str, Any]:
        ...
        try:
            ...
            analysis_results["issues"].append(
                {"type": "analysis_error", "component": "documentation", "message": str(e)}
            )
        ...
        if self.repo_analyzer:
            ...
            try:
                ...
                analysis_results["issues"].append(
                    {"type": "analysis_error", "component": "repository", "message": str(e)}
                )
        ...
        if self.github_client:
            ...
            try:
                ...
                analysis_results["issues"].append({"type": "analysis_error", "component": "github", "message": str(e)})
        ...
        try:
            ...
            analysis_results["issues"].append({"type": "analysis_error", "component": "integration", "message": str(e)})
    ...
    def _analyze_integration(self, analysis_results: dict[str, Any]) -> dict[str, Any]:
        ...
        if analysis_results.get("documentation_analysis") and analysis_results.get("repository_analysis"):
            integration_data["documentation_code_alignment"] = self._analyze_doc_code_alignment(
                analysis_results["documentation_analysis"], analysis_results["repository_analysis"]
            )
        ...
        if analysis_results.get("github_analysis") and analysis_results.get("documentation_analysis"):
            integration_data["github_documentation_correlation"] = self._analyze_github_doc_correlation(
                analysis_results["github_analysis"], analysis_results["documentation_analysis"]
            )
    ...
    def _analyze_github_doc_correlation(self, github_results: dict, doc_results: dict) -> dict[str, Any]:
        ...
        try:
            ...
            doc_prs = [
                pr
                for pr in prs
                if any(keyword in pr.get("title", "").lower() for keyword in ["doc", "readme", "guide"])
            ]
            if prs:
            ...
            correlation_data["maintenance_activity"] = (
                activity_metrics.get("issue_close_rate", 0) * 50 + activity_metrics.get("pr_merge_rate", 0) * 50
            )
            ...
            for gap in gaps:
                if gap.get("severity") == "high":
                    recommendations["high_priority"].append(
                        {"title": gap["title"], "description": gap["description"], "actions": [gap["recommendation"]]}
                    )
...
def main():
    ...
    # Print summary
    results.get("combined_metrics", {})

    # Show critical issues
    issues = results.get("issues", [])
    if issues:
        for _issue in issues[:3]:  # Show first 3
            pass

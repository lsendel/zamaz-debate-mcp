...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("kiro_github_events.log")],
)
...
def get_github_client(installation_id):
    ...
    if not token:
        ...
        now = int(time.time())
        payload = {"iat": now, "exp": now + (10 * 60), "iss": credentials["app_id"]}

        try:
            jwt_token = jwt.encode(payload, credentials["private_key"], algorithm="RS256")
        except Exception as e:
            logger.error(f"Error generating JWT: {e!s}")
            return None
        ...
        try:
            ...
            logger.error(f"Error getting installation token: {e!s}")
            return None
...
def process_pr_for_review(pr, repository, installation_id, is_update=False):
    ...
    try:
        ...
        logger.error(f"Error processing PR for review: {e!s}")
...
def process_suggestion_command(comment, issue, repository, installation_id):
    ...
    try:
        ...
        success = handle_suggestion_comment(github, repo_full_name, pr_number, comment_id, comment_body, config)

        # Add a reply to the comment
        reply_url = f"{GITHUB_API_URL}/repos/{repo_full_name}/issues/{pr_number}/comments"

        if success:
            reply_body = "✅ Successfully applied the suggestion! The changes have been committed to the PR branch."
        else:
            reply_body = "❌ Failed to apply the suggestion. Please check the syntax or try applying it manually."

        reply_data = {"body": reply_body, "in_reply_to": comment_id}

        github.post(reply_url, json=reply_data)
        logger.info(f"Added reply to suggestion command in comment #{comment_id}")

    except Exception as e:
        logger.error(f"Error processing suggestion command: {e!s}")

        # Add error reply
        try:
            reply_url = f"{GITHUB_API_URL}/repos/{repo_full_name}/issues/{pr_number}/comments"
            reply_data = {"body": f"❌ Error applying suggestion: {e!s}", "in_reply_to": comment_id}
            github.post(reply_url, json=reply_data)
        except:
...
def mentions_kiro(text):
    ...
    # Check for commands
    kiro_commands = [r"/kiro\s+review", r"/kiro\s+help", r"/kiro\s+explain", r"/kiro\s+fix", r"/kiro\s+ignore"]

    return any(re.search(command, text, re.IGNORECASE) for command in kiro_commands)

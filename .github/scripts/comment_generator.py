...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('kiro_comment_generator.log')
    ]
)
...
class CommentGenerator:
    ...
    def _generate_comment_for_line(self, file_path: str, line_number: int, issues: List[Dict[str, Any]]) -> str:
        ...
        for i, issue in enumerate(issues):
            ...
            category = issue.get('category', 'best_practice').replace('_', ' ').title()
...
def generate_comments(issues: List[Dict[str, Any]], config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Generate comments for a list of issues."""
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: comment_generator.py <issues_file>")
        sys.exit(1)
    ...
    try:
        ...
        print("Summary Comment:")
        print(comments['summary_comment'])
        print("\nFile Comments:")
        for comment in comments['file_comments']:

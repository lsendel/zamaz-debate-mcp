...
from ..core.interfaces import CacheInterface, DatabaseInterface, GitHubClientInterface, MetricsInterface, QueueInterface
from ..core.logging import get_logger
from ..diagnostics.diagnostic_collector import DiagnosticCollector, ErrorAnalyzer, PerformanceProfiler, RequestTracer
...
def get_request_tracer(collector: DiagnosticCollector = Depends(get_diagnostic_collector)) -> RequestTracer:
...
async def detailed_health_check(container: ServiceContainer = Depends()) -> dict[str, Any]:
    """
    ...
    try:
        ...
        health_status["errors"].append(
            {"component": "database", "error": str(e), "timestamp": datetime.utcnow().isoformat()}
        )
    ...
    try:
        ...
        health_status["components"]["cache"] = {"status": "healthy", "latency_ms": 0}
    except Exception as e:
        health_status["status"] = "degraded"
    ...
    try:
        ...
        health_status["components"]["queue"] = {"status": "healthy", "size": queue_size}
    except Exception as e:
        health_status["status"] = "degraded"
        health_status["components"]["queue"] = {"status": "unhealthy", "error": str(e)}
    ...
    try:
        ...
        health_status["components"]["github_api"] = {"status": "degraded", "error": str(e)}
    ...
    # Overall status
    unhealthy_components = [name for name, info in health_status["components"].items() if info["status"] != "healthy"]
...
async def get_system_state(collector: DiagnosticCollector = Depends(get_diagnostic_collector)) -> dict[str, Any]:
...
async def get_diagnostic_data(
    correlation_id: str, collector: DiagnosticCollector = Depends(get_diagnostic_collector)
) -> dict[str, Any]:
...
async def get_request_trace(correlation_id: str, tracer: RequestTracer = Depends(get_request_tracer)) -> dict[str, Any]:
    ...
    return {"correlation_id": correlation_id, "trace": trace, "timeline": timeline}


@router.get("/errors/summary")
async def get_error_summary(analyzer: ErrorAnalyzer = Depends(get_error_analyzer)) -> dict[str, Any]:
    """Get error analysis summary."""
    return analyzer.get_error_summary()


@router.get("/performance/summary")
async def get_performance_summary(profiler: PerformanceProfiler = Depends(get_performance_profiler)) -> dict[str, Any]:
    """Get performance profiling summary."""
    summary = profiler.get_performance_summary()
    bottlenecks = profiler.identify_bottlenecks()

    return {"operations": summary, "bottlenecks": [{"operation": op, "p95_ms": ms} for op, ms in bottlenecks]}


@router.get("/metrics/current")
async def get_current_metrics(container: ServiceContainer = Depends()) -> dict[str, Any]:
...
async def test_component_connection(component: str, container: ServiceContainer = Depends()) -> dict[str, Any]:
...
async def cleanup_old_diagnostics(
    older_than_hours: int = Query(default=24, ge=1, le=168), container: ServiceContainer = Depends()
) -> dict[str, Any]:
    """Clean up old diagnostic data."""
    database = container.resolve(DatabaseInterface)
    container.resolve(CacheInterface)

    cutoff_time = datetime.utcnow() - timedelta(hours=older_than_hours)

    # Clean database
    db_result = await database.execute(
        "DELETE FROM diagnostics WHERE created_at < ?", {"created_at": cutoff_time.isoformat()}
    )

...
from ..core.exceptions import ConfigurationError, WebhookProcessingError, WebhookValidationError
from ..core.interfaces import CacheInterface, GitHubClientInterface, MetricsInterface, QueueInterface
...
class WebhookValidator:
    ...
    def validate_signature(self, payload: bytes, signature: str) -> bool:
        ...
        expected_signature = "sha256=" + hmac.new(self.secret, payload, hashlib.sha256).hexdigest()
...
class WebhookService:
    ...
    async def process_webhook(self, headers: dict[str, str], body: bytes) -> dict[str, Any]:
        ...
        # Record metrics
        self.metrics.increment("webhook.received", tags={"event": event_type, "action": event.action or "none"})
    ...
    async def _process_event(self, event: WebhookEvent) -> dict[str, Any]:
        ...
        if not handler:
            ...
            return {"status": "ignored", "reason": "unsupported_event", "event_type": event.event_type}

        try:
            result = await handler(event)

            self.metrics.increment("webhook.processed", tags={"event": event.event_type, "status": "success"})

            return {"status": "processed", "event_type": event.event_type, "result": result}

        except Exception as e:
            logger.error(f"Error processing webhook: {e}", exc_info=True)

            self.metrics.increment("webhook.processed", tags={"event": event.event_type, "status": "error"})
    ...
    async def _handle_pull_request(self, event: WebhookEvent) -> dict[str, Any]:
        ...
        message_id = await self.queue.publish("pr_processing", pr_data, priority=self._get_priority(pr_data))

        return {"processed": True, "message_id": message_id, "pr_number": pr["number"]}
    ...
    async def _handle_issue_comment(self, event: WebhookEvent) -> dict[str, Any]:
        ...
        if "@kiro" in comment["body"].lower():
            ...
            message_id = await self.queue.publish("comment_processing", comment_data)

            return {"processed": True, "message_id": message_id, "comment_id": comment["id"]}
    ...
    async def _should_process_pr(self, pr_data: dict[str, Any]) -> bool:
        ...
        # Check PR title
        return "[kiro]" in pr.get("title", "").lower()

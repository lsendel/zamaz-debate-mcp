...
class StructuredLogger:
    ...
    def _setup_json_formatter(self):
        ...
        formatter = CorrelationJsonFormatter("%(timestamp)s %(level)s %(name)s %(message)s")
    ...
    def metric(self, metric_name: str, value: int | float, **kwargs):
        """Log metric data."""
        extra = self._add_context(kwargs)
...
class LogContext:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type:
            self.logger.error(f"Failed {self.operation}", error=exc_val, duration_ms=duration_ms, **self.extra)
        else:
            self.logger.info(f"Completed {self.operation}", duration_ms=duration_ms, **self.extra)
...
class PerformanceLogger:
    ...
    def record_metric(self, name: str, value: int | float, **kwargs):

...
class GitHubCredentialManager:
    ...
    def __init__(self, storage_path=None, encryption_key=None):
        ...
        self.storage_path = storage_path or os.environ.get(
            'GITHUB_CREDENTIAL_STORAGE',
            str(Path.home() / '.kiro' / 'credentials' / 'github.enc')
        )
        ...
        if not self.encryption_key:
            ...
            self.encryption_key = base64.urlsafe_b64encode(
                kdf.derive(b'kiro-default-key')
            )
            print("Warning: Using default encryption key. Set GITHUB_CREDENTIAL_KEY for production.")
    ...
    def store_installation_token(self, installation_id, token, expires_at):
        ...
        credentials['installations'][str(installation_id)] = {
            'token': token,
...
def setup_credentials_from_env():
    ...
    if all([app_id, client_id, client_secret, private_key, webhook_secret]):
        ...
        missing = []
        if not app_id: missing.append("GITHUB_APP_ID")
        if not client_id: missing.append("GITHUB_CLIENT_ID")
        if not client_secret: missing.append("GITHUB_CLIENT_SECRET")
        if not private_key: missing.append("GITHUB_PRIVATE_KEY_PATH or GITHUB_PRIVATE_KEY_BASE64")
        if not webhook_secret: missing.append("GITHUB_WEBHOOK_SECRET")
...
if __name__ == "__main__":
    ...
    if setup_credentials_from_env():
        print("Successfully stored GitHub App credentials")
    else:
        print("Failed to store GitHub App credentials")
...
class SlackIntegration:
    ...
    async def send_review_notification(
        ...
        try:
            # Build review blocks
            blocks = self._build_review_blocks(repository, pull_request, review_summary, author, pr_title)

            # Send message
            response = await self.client.chat_postMessage(
                channel=channel, text=f"Code review completed for {repository} PR #{pull_request}", blocks=blocks
            )
    ...
    def _build_review_blocks(
        self, repository: str, pull_request: int, review_summary: dict[str, Any], author: str, pr_title: str
    ) -> list[dict[str, Any]]:
        ...
        blocks = [
            {"type": "header", "text": {"type": "plain_text", "text": f"{status_emoji} Code Review Complete"}},
            {
                "type": "section",
                "fields": [
            ...
            {"type": "divider"},
            {"type": "section", "text": {"type": "mrkdwn", "text": f"*Review Summary:* {status_text}"}},
            {
                "type": "section",
                "fields": [
                    {"type": "mrkdwn", "text": f"*Total Issues:*\n{total_issues}"},
                    {"type": "mrkdwn", "text": f"*Critical Issues:*\n{critical_issues}"},
                    {"type": "mrkdwn", "text": f"*Suggestions:*\n{suggestions}"},
        ...
        blocks.append(
            {
                ...
                "elements": [
                    ...
                    },
                    {
                        ...
                        "value": json.dumps({"repository": repository, "pull_request": pull_request}),
                    },
                    {
                        "type": "button",
    ...
    async def send_suggestion_card(self, channel: str, suggestion: dict[str, Any], context: dict[str, Any]) -> bool:
        """Send an interactive suggestion card."""
        try:
            blocks = [
                {"type": "header", "text": {"type": "plain_text", "text": "💡 Kiro Suggestion"}},
                {
                    "type": "section",
                    "text": {
                        "type": "mrkdwn",
                ...
                {
                    "type": "section",
                    "text": {"type": "mrkdwn", "text": f"```{context['language']}\n{context['code_snippet']}\n```"},
                },
                {"type": "section", "text": {"type": "mrkdwn", "text": f"*Suggestion:*\n{suggestion['message']}"}},
                {
                    "type": "section",
                    "text": {
                        "type": "mrkdwn",
            ...
            response = await self.client.chat_postMessage(
                channel=channel, text="New code suggestion from Kiro", blocks=blocks
            )
...
• Customize notification settings
        ...
        await client.chat_postEphemeral(channel=user_id, user=user_id, text=help_text)
    ...
    async def handle_review_command(self, ack, command, client):
        ...
        await client.chat_postMessage(
            channel=command["channel_id"], text=f"🔍 Initiating review for {repository} PR #{pr_number}..."
        )
        ...
        await self.send_review_notification(
            ...
            repository,
            int(pr_number),
            {"total_issues": 3, "critical_issues": 0, "suggestions_made": 5, "time_saved": "15 minutes"},
            command["user_name"],
            "Sample PR Title",
        )
    ...
    async def handle_stats_command(self, ack, command, client):
        ...
        blocks = [
            {"type": "header", "text": {"type": "plain_text", "text": "📊 Kiro Statistics"}},
            {
                "type": "section",
                "text": {
                    "type": "mrkdwn",
    ...
    async def handle_approve_suggestion(self, ack, body, client):
        ...
        blocks[-1] = {
            "type": "section",
            "text": {"type": "mrkdwn", "text": f"✅ *Suggestion approved by {user}*\nFix has been applied to the PR."},
        }

        await client.chat_update(channel=body["channel"]["id"], ts=body["message"]["ts"], blocks=blocks)
    ...
    async def handle_reject_suggestion(self, ack, body, client):
        ...
        blocks = body["message"]["blocks"]
        blocks[-1] = {"type": "section", "text": {"type": "mrkdwn", "text": f"❌ *Suggestion dismissed by {user}*"}}
        ...
        await client.views_open(
            ...
            view={
                ...
                "blocks": [
                    ...
                    {
                        "type": "section",
                        "text": {"type": "mrkdwn", "text": "Detailed review information would be displayed here..."},
                    },
                ],
            },
        )
    ...
    async def handle_configure_notifications(self, ack, body, client):
        ...
        await client.views_open(
            ...
            view={
                ...
                "title": {"type": "plain_text", "text": "Configure Kiro"},
                "submit": {"type": "plain_text", "text": "Save"},
                "blocks": [
                    {
                        "type": "section",
                        "text": {"type": "mrkdwn", "text": f"Configure notifications for *{repository}*"},
                    },
                    {
                        "type": "input",
                        ...
                        "element": {
                            ...
                            "options": [
                                {"text": {"type": "plain_text", "text": "All Issues"}, "value": "all"},
                                {"text": {"type": "plain_text", "text": "Critical Only"}, "value": "critical"},
                                {"text": {"type": "plain_text", "text": "Summary Only"}, "value": "summary"},
                            ],
                        },
                    },
                    {
                        "type": "input",
                        "block_id": "auto_fix",
                        "label": {"type": "plain_text", "text": "Auto-apply Safe Fixes"},
                        "element": {
                            "type": "checkboxes",
                            "action_id": "auto_fix_check",
                            "options": [
                                {"text": {"type": "plain_text", "text": "Enable auto-fix"}, "value": "enabled"}
                            ],
                        },
                    },
                ],
    ...
    async def handle_configuration_submission(self, ack, body, view, client):
        ...
        # Send confirmation
        await client.chat_postMessage(channel=body["user"]["id"], text=f"✅ Configuration saved for {repository}")

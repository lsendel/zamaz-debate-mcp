...
Examples:
  ...
  python run_documentation_analysis.py /path/to/project -v
    ...
    parser.add_argument(
        '--search', '-s',
        help='Search query to test documentation search functionality'
    )
    ...
    parser.add_argument(
        '--quick',
        action='store_true',
        help='Run quick analysis (skip some time-consuming tasks)'
    )
    
    parser.add_argument(
        '--docs-only',
        action='store_true',
        help='Analyze only documentation files (skip code analysis)'
    )
    
    parser.add_argument(
        '--code-only',
        action='store_true',
        help='Analyze only code files (skip documentation parsing)'
    )
    ...
    try:
        ...
        print(f"Cross-References: {metrics.get('cross_reference_count', 0)}")
        ...
        if recommendations:
            ...
            for category in ['high_priority', 'medium_priority']:
                if recommendations.get(category):
                    ...
                    for rec in recommendations[category][:3]:  # Show top 3
                        print(f"  - {rec['title']}")

...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('kiro_learning.log')
    ]
)
...
class LearningSystem:
    ...
    def __init__(self, analytics_db_path: str = None, learning_db_path: str = None):
    ...
    def update_rule_effectiveness(self, rule_id: str, category: str, feedback_type: str, suggestion_applied: bool = False):
        ...
        try:
            ...
            if result:
                ...
                total_occurrences, positive_feedback, negative_feedback, suggestions_applied, suggestions_rejected, current_score = result
                ...
                SET total_occurrences = ?, positive_feedback = ?, negative_feedback = ?,
                    ...
                    last_updated = ?
                WHERE rule_id = ? AND category = ?
                ''', (total_occurrences, positive_feedback, negative_feedback,
                      suggestions_applied, suggestions_rejected, effectiveness_score,
                      datetime.now().isoformat(), rule_id, category))
            else:
                # Create new record
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (rule_id, category, 1, positive_feedback, negative_feedback,
                      suggestions_applied, suggestions_rejected, effectiveness_score,
            ...
            logger.error(f"Error updating rule effectiveness: {str(e)}")
            return False
    
    def _calculate_effectiveness_score(self, positive_feedback: int, negative_feedback: int, 
                                     suggestions_applied: int, suggestions_rejected: int) -> float:
    ...
    def learn_pattern(self, pattern_type: str, pattern_value: str, file_extension: str, success: bool):
        ...
        try:
            ...
            if result:
                ...
                WHERE pattern_type = ? AND pattern_value = ? AND file_extension = ?
                ''', (success_count, failure_count, confidence_score, datetime.now().isoformat(),
                      pattern_type, pattern_value, file_extension))
            else:
                # Create new record
    ...
    def update_developer_preferences(self, developer: str, repo_owner: str, repo_name: str, 
                                   rule_id: str, preference_score: float):
        ...
        try:
            ...
            if result:
                ...
                WHERE developer = ? AND repo_owner = ? AND repo_name = ? AND rule_id = ?
                ''', (new_preference_score, interaction_count, datetime.now().isoformat(),
                      developer, repo_owner, repo_name, rule_id))
            else:
                # Create new record
                cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
                ''', (developer, repo_owner, repo_name, rule_id, preference_score, 1,
                      datetime.now().isoformat()))
            ...
            logger.error(f"Error updating developer preferences: {str(e)}")
            return False
    
    def get_rule_recommendations(self, repo_owner: str, repo_name: str, developer: str = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error getting rule recommendations: {str(e)}")
            return []
    ...
    def get_pattern_confidence(self, pattern_type: str, pattern_value: str, file_extension: str) -> float:
        ...
        try:
            ...
            logger.error(f"Error getting pattern confidence: {str(e)}")
            return 0.5
    ...
    def improve_suggestion(self, original_suggestion: str, context: Dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.error(f"Error improving suggestion: {str(e)}")
    ...
    def record_suggestion_improvement(self, original_suggestion: str, improved_suggestion: str, 
                                   improvement_type: str, success: bool):
        ...
        try:
            ...
            if result:
                ...
                WHERE original_suggestion = ? AND improved_suggestion = ?
                ''', (new_success_rate, usage_count, datetime.now().isoformat(),
                      original_suggestion, improved_suggestion))
            else:
                # Create new record
                ...
                VALUES (?, ?, ?, ?, ?, ?)
                ''', (original_suggestion, improved_suggestion, improvement_type, success_rate, 1,
                      datetime.now().isoformat()))
            ...
            logger.error(f"Error recording suggestion improvement: {str(e)}")
            return False
    
    def get_learning_insights(self, repo_owner: str = None, repo_name: str = None) -> Dict[str, Any]:
...
def update_developer_preferences(developer: str, repo_owner: str, repo_name: str, rule_id: str, preference_score: float):
    """Update developer preferences for specific rules."""
    system = LearningSystem()
    return system.update_developer_preferences(developer, repo_owner, repo_name, rule_id, preference_score)

def get_rule_recommendations(repo_owner: str, repo_name: str, developer: str = None) -> List[Dict[str, Any]]:
    """Get rule recommendations based on effectiveness and preferences."""
    system = LearningSystem()
...
def get_learning_insights(repo_owner: str = None, repo_name: str = None) -> Dict[str, Any]:
    """Get insights from the learning system."""
    system = LearningSystem()
...
if __name__ == "__main__":
    ...
    for rec in recommendations[:5]:
        print(f"  {rec['rule_id']}: {rec['combined_score']:.2f}")

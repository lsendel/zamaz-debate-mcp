...
from ..core.interfaces import AnalyzerInterface, AnalysisResult, CodeIssue, IssueLevel
from ..core.exceptions import AnalysisError, FileNotSupportedError, AnalysisTimeoutError
        ...
        for strategy in self.strategies:
            ...
            try:
                ...
                issues = await asyncio.wait_for(
                    strategy.analyze(analysis_context),
                    timeout=analysis_context.timeout_seconds
                )
                ...
                strategy_results[strategy.get_name()] = f"error: {str(e)}"
        
        # Sort issues by severity and line number
        all_issues.sort(key=lambda x: (
            self._severity_order(x.level),
            x.line_number or 0
        ))

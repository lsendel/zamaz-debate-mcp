...
try:
    ...
    print("Warning: Some optional dependencies are missing. Basic functionality will be available.")
...
class JavaDocExtractor:
    ...
    def extract(self, content: str, file_path: str) -> CodeFile:
        ...
        for match in matches:
            code_file.javadoc_comments.append({
                'content': match.strip(),
                'tags': self._extract_javadoc_tags(match)
            })
    ...
    def extract(self, content: str, file_path: str) -> CodeFile:
        ...
        for match in matches:
            code_file.comments.append({
                'type': 'jsdoc',
                'content': match.strip(),
                'tags': self._extract_jsdoc_tags(match)
            })
        ...
        for pattern in endpoint_patterns:
            ...
            for match in matches:
                code_file.api_endpoints.append({
                    'method': match if isinstance(match, str) else match[0],
                    'type': 'endpoint'
                })
    ...
    def extract(self, content: str, file_path: str) -> CodeFile:
        ...
        for match in matches:
            code_file.comments.append({
                'type': 'tsdoc',
                'content': match.strip(),
                'tags': self._extract_tsdoc_tags(match)
            })
    ...
    def analyze_coverage(self, project_path: str) -> Dict[str, Any]:
        ...
        for file_path in code_files:
            try:
                ...
                has_docs = (
                    code_file.documentation_coverage > 0 or
                    len(code_file.comments) > 0 or
                    len(code_file.docstrings) > 0
                )
                ...
                if file_type not in coverage_data['coverage_by_type']:
                    coverage_data['coverage_by_type'][file_type] = {
                        'total': 0,
                        'documented': 0,
                        'coverage': 0.0
                    }
        ...
        if coverage_data['total_files'] > 0:
            coverage_data['overall_coverage'] = (
                coverage_data['documented_files'] / coverage_data['total_files']
            ) * 100
...
class OutdatedDocumentationDetector:
                        ...
                        if os.path.exists(doc_path):
                            ...
                            if doc_mtime < code_mtime:
                                issues.append(DocumentationIssue(
                                    severity='warning',
                                    type='outdated',
                                    message=f'Documentation may be outdated (code modified more recently)',
                                    file_path=doc_path,
    ...
    def _find_related_docs(self, code_file: str, project_path: str) -> List[str]:
        ...
        for root, dirs, files in os.walk(os.path.join(project_path, 'docs')):
            for file in files:
                    ...
                    try:
                        ...
                        if code_file.api_endpoints:
                            api_endpoints.extend([
                                (file_path, endpoint) for endpoint in code_file.api_endpoints
                            ])
                    except Exception as e:
                        logger.warning(f"Error analyzing {file_path}: {e}")
...
class DocumentationAnalysisSystem:
    ...
    def _analyze_code_files(self, project_path: str) -> List[CodeFile]:
        ...
        for root, dirs, files in os.walk(project_path):
            # Skip certain directories
            dirs[:] = [d for d in dirs if d not in {'.git', 'node_modules', 'target', 'build', '.gradle', 'venv', '__pycache__'}]
    ...
    def _calculate_metrics(self, docs: List[DocumentationFile], code_files: List[CodeFile]) -> DocumentationMetrics:
        ...
        documented_code_items = sum(
            sum(1 for item in cf.classes + cf.functions 
                if item.get('documentation') or item.get('docstring'))
...
def main():
    ...
    if args.search:
        ...
        for doc, score in search_results:
            print(f"- {doc.path} (score: {score:.3f})")

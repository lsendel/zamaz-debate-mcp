...
from ...scripts.analyzers.style_analyzer import ComplexityStrategy, StyleGuideStrategy
...
class PerformanceMetrics:
    ...
    def record_operation(self, name: str, duration_ms: float, metadata: dict | None = None):
        """Record an operation's performance."""
        self.operations.append(
            {"name": name, "duration_ms": duration_ms, "timestamp": datetime.utcnow(), "metadata": metadata or {}}
        )
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        for op in self.operations:
            ...
            if name not in operation_stats:
                operation_stats[name] = {"count": 0, "total_ms": 0, "min_ms": float("inf"), "max_ms": 0}
...
class TestPRProcessingPerformance:
    ...
    async def test_concurrent_pr_processing(self, performance_metrics):
        ...
        async def process_pr(pr_number: int):
            ...
            performance_metrics.record_operation("process_pr", duration, {"pr_number": pr_number})
    ...
    async def test_large_pr_performance(self, performance_metrics):
        ...
        large_file_content = "\n".join(
            [
                f"def function_{i}():    # This is line {i}    return {i}"
                for i in range(1000)  # 3000 lines
            ]
        )

        # Analyze large file
        analyzer = BaseAnalyzer([SecurityPatternStrategy(), StyleGuideStrategy(), ComplexityStrategy()])

        start = time.time()
        result = await analyzer.analyze("large_file.py", large_file_content)
...
"""
        ...
        context = AnalyzerContext(file_path="test.py", content=test_code, language="python")
        ...
        for name, strategy in strategies:
            ...
            performance_metrics.record_operation(f"analyze_{name.lower()}", duration, {"issues_found": len(issues)})
        ...
        for _op_name, stats in summary["operations"].items():

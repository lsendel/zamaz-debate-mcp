...
class TestLoadScenarios:
    ...
    def generate_pr_payload(self, pr_number: int, num_files: int = 5) -> Dict[str, Any]:
        ...
        return {
            ...
            'repository': {
                'full_name': f'testorg/repo{pr_number % 5}',
                'name': f'repo{pr_number % 5}'
            },
            'files': files
        }
    ...
    async def test_concurrent_webhook_processing(self):
        ...
        # Generate webhooks
        webhooks = [
            self.generate_pr_payload(i, random.randint(1, 20))
        ...
        for i in range(0, num_webhooks, webhooks_per_second):
            ...
            batch_results = await asyncio.gather(
                *[process_webhook(w) for w in batch]
            )
        ...
        print(f"\nLoad Test Results:")
        print(f"Total webhooks processed: {len(results)}")
        ...
        print(f"Throughput: {len(results) / total_time:.2f} webhooks/second")
    ...
    async def test_large_pr_processing_performance(self):
        ...
        for size in pr_sizes:
            ...
            print(f"PR with {size} files: {processing_time:.2f}s")
    ...
    async def test_api_rate_limit_handling(self):
        ...
        for i in range(100):
            try:
                ...
                print(f"Rate limited at request {i}, waiting {rate_limit_reset_time}s...")
        ...
        print(f"Rate limit errors: {len(errors)}")
    ...
    async def test_concurrent_repo_processing(self):
        ...
        print(f"Total time: {end_time - start_time:.2f}s")
        print(f"Throughput: {len(results) / (end_time - start_time):.2f} PRs/second")
    ...
    async def test_spike_load_handling(self):
        ...
        for r in results:
            status = "SPIKE" if r['load'] == spike_load else "normal"
            print(f"Second {r['second']} ({status}): "
                  f"{r['successful']}/{r['load']} successful, "
        ...
        print(f"\nSpike success rate: {success_rate:.2%}")
    ...
    async def process_pr_with_timeout(self, pr_data, timeout=2.0):
        ...
        try:
            return await asyncio.wait_for(
                self.simulate_pr_processing(pr_data),
                timeout=timeout
            )
    ...
    async def simulate_pr_processing(self, pr_data):
        ...
        return {
            ...
            'issues_found': random.randint(0, num_files * 2)
        }
        ...
        for concurrent_ops in range(100, max_concurrent + 1, step_size):
            try:
                ...
                if successful < concurrent_ops * 0.9:
                    print(f"System limit reached at {concurrent_ops} concurrent operations")
                    break
                    
            except Exception as e:
                print(f"System failed at {concurrent_ops} concurrent operations: {e}")
                break

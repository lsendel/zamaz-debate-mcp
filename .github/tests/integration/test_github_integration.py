...
class TestGitHubIntegrationFlow:
    ...
    def sample_pr_webhook_payload(self):
        ...
        return {
            ...
            "pull_request": {
                ...
                "head": {"sha": "abc123", "ref": "feature/auth", "repo": {"full_name": "testorg/testrepo"}},
                "base": {"sha": "def456", "ref": "main", "repo": {"full_name": "testorg/testrepo"}},
    ...
    async def test_complete_pr_review_flow(
        self, webhook_handler, pr_processor, code_analyzer, comment_generator, github_client, sample_pr_webhook_payload
    ):
        ...
        json.loads(webhook_handler.redis_client.rpush.call_args[0][1])
        ...
        github_client.get_pull_request = AsyncMock(
            return_value={
                ...
                "commits": [{"sha": "abc123", "commit": {"message": "Add authentication function"}}],
            }
        )

        pr_data = await pr_processor.fetch_pr_data("testorg/testrepo", 42)
        ...
        for file in pr_data["files"]:
            if file["patch"]:
                result = await code_analyzer.analyze_diff(file["patch"], file["filename"], "python")
        ...
        for analysis in analysis_results:
            for issue in analysis.issues:
                ...
                comments.append({"path": analysis.file_path, "line": issue.line or 1, "body": comment})

        assert len(comments) > 0
        assert "TODO" in comments[0]["body"] or "validation" in comments[0]["body"]

        # Step 5: GitHub Client posts the review
        github_client.create_pull_request_review = AsyncMock(
            return_value={"id": 123456, "state": "COMMENTED", "body": "Automated review by Kiro"}
        )

        review_result = await github_client.create_pull_request_review(
            "testorg/testrepo",
            42,
            {"body": "Kiro has reviewed your pull request.", "event": "COMMENT", "comments": comments},
        )
...
"""
        ...
        analysis = await code_analyzer.analyze_code(vulnerable_code, "database.py", "python")
    ...
    @pytest.mark.asyncio
    async def test_pr_with_multiple_files(self, pr_processor, code_analyzer, github_client):
        ...
        github_client.get_pull_request = AsyncMock(
            return_value={
                "files": [
                    ...
                    {"filename": "README.md", "status": "modified", "patch": "@@ -1,0 +1,1 @@\n+# Project Title"},
                ]
            }
        )
        ...
        for file in pr_data["files"]:
            ...
            result = await code_analyzer.analyze_diff(file["patch"], file["filename"], language)
...
"""
        ...
        result = await analyzer.analyze_code(code_with_style_issues, "test.py", "python")

        # Should not report style issues if disabled
        [i for i in result.issues if i.type.value == "style"]
    ...
    async def test_comment_batching(self, comment_generator):
        ...
        issues = [
            Mock(type=Mock(value="style"), severity=Mock(value="minor"), message="Line too long", line=10),
            Mock(type=Mock(value="style"), severity=Mock(value="minor"), message="Missing docstring", line=15),
            Mock(type=Mock(value="security"), severity=Mock(value="critical"), message="Hardcoded password", line=20),
        ]
    ...
    async def test_incremental_review_on_push(self, webhook_handler, _pr_processor, _code_analyzer):
        ...
        event = WebhookEvent(event_type=EventType.PULL_REQUEST, action="synchronize", payload=push_payload)
...
"""
        ...
        analysis = await code_analyzer.analyze_code(code_with_issue, "processor.py", "python")

        # Find unused import issue
        unused_import_issues = [
            i for i in analysis.issues if "unused" in i.message.lower() and "import" in i.message.lower()
        ]
...
"""
        ...
        analysis = await code_analyzer.analyze_code(inefficient_code, "utils.py", "python")

        performance_issues = [i for i in analysis.issues if i.type.value == "performance"]

...
class MockGitHubClient(GitHubClientInterface):
    ...
    def __init__(self):
        ...
        self.comments = {}
        self.rate_limit = {
            "rate": {"limit": 5000, "remaining": 4999, "reset": int(datetime.utcnow().timestamp()) + 3600}
        }
...
class TestGitHubWebhookIntegration:
    ...
    async def test_pr_assignment_flow(self, mock_github_client):
        ...
        service = WebhookService(
            validator=validator, queue=AsyncMock(), cache=AsyncMock(), metrics=Mock(), github_client=mock_github_client
        )
    ...
    async def test_review_creation_flow(self, mock_github_client):
        ...
        review_data = {
            ...
            "comments": [{"path": "src/main.py", "position": 10, "body": "Consider adding error handling here"}],
        }

        # Create review
        result = await mock_github_client.create_review("test-org", "test-repo", 123, review_data)
...
"""
    ...
    async def test_review_comment_generation(self, mock_github_client):
        ...
        review_data = {
            ...
            if any(i["level"] == "error" for issues in analysis_results.values() for i in issues)
            else "COMMENT",
...
class TestWebhookScenarios:
    ...
    async def test_review_requested_event(self, _mock_github_client):
        ...
        payload = {
            ...
            "pull_request": {"number": 123, "requested_reviewers": [{"login": "kiro-ai"}]},
            "repository": {"owner": {"login": "test-org"}, "name": "test-repo"},
        }
    ...
    async def test_comment_mention_event(self, _mock_github_client):
        ...
        payload = {
            ...
            "comment": {"body": "@kiro-ai can you review this security fix?", "user": {"login": "developer1"}},
            "repository": {"owner": {"login": "test-org"}, "name": "test-repo"},
        }
...
async def integration_test_setup():
    ...
    config = {"github_app_id": "test-app-123", "webhook_secret": "test-secret", "processing_timeout": 30}

    return {"review_repo": review_repo, "config": config}
...
class TestFullIntegration:
    ...
    async def test_complete_pr_review_flow(self, mock_github_client, integration_test_setup):
        ...
        # 4. Get PR files
        await mock_github_client.get_pr_files("test-org", "test-repo", 123)
        ...
        # 6. Update review
        await setup["review_repo"].update(
            review_id,
            {
                "status": "completed",
        ...
        review_result = await mock_github_client.create_review(
            "test-org", "test-repo", 123, {"event": "COMMENT", "body": f"Found {issues_found} issues", "comments": []}
        )

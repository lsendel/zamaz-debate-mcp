...
class TestWebhookHandler:
    ...
    async def test_handle_pull_request_opened(self, webhook_handler, sample_pr_event):
        ...
        event = WebhookEvent(event_type=EventType.PULL_REQUEST, action="opened", payload=sample_pr_event)
    ...
    async def test_handle_pull_request_synchronize(self, webhook_handler, sample_pr_event):
        ...
        event = WebhookEvent(event_type=EventType.PULL_REQUEST, action="synchronize", payload=sample_pr_event)
    ...
    async def test_handle_pull_request_closed(self, webhook_handler, sample_pr_event):
        ...
        event = WebhookEvent(event_type=EventType.PULL_REQUEST, action="closed", payload=sample_pr_event)
    ...
    async def test_handle_pull_request_review_requested(self, webhook_handler, sample_pr_event):
        ...
        sample_pr_event["requested_reviewer"] = {"login": "reviewer1", "id": 999}
        webhook_handler.redis_client.rpush = AsyncMock()

        event = WebhookEvent(event_type=EventType.PULL_REQUEST, action="review_requested", payload=sample_pr_event)
    ...
    async def test_handle_issue_comment_with_mention(self, webhook_handler):
        ...
        payload = {
            "action": "created",
            "issue": {"number": 42, "pull_request": {"url": "https://api.github.com/repos/testorg/test-repo/pulls/42"}},
            "comment": {"body": "@kiro please review this", "user": {"login": "testuser"}},
            "repository": {"full_name": "testorg/test-repo"},
        }

        event = WebhookEvent(event_type=EventType.ISSUE_COMMENT, action="created", payload=payload)
    ...
    async def test_handle_issue_comment_without_mention(self, webhook_handler):
        ...
        payload = {
            ...
            "comment": {"body": "Just a regular comment", "user": {"login": "testuser"}},
            "repository": {"full_name": "testorg/test-repo"},
        }
    ...
    async def test_handle_installation_created(self, webhook_handler):
        ...
        payload = {
            "action": "created",
            "installation": {"id": 12345, "account": {"login": "testorg", "type": "Organization"}},
            "repositories": [
        ...
        event = WebhookEvent(event_type=EventType.INSTALLATION, action="created", payload=payload)
    ...
    async def test_parse_webhook_valid(self, webhook_handler):
        ...
        headers = {"X-GitHub-Event": "pull_request", "X-GitHub-Delivery": "test-delivery-id"}

        body = {"action": "opened", "pull_request": {"id": 1}, "repository": {"name": "test"}}
    ...
    async def test_parse_webhook_unknown_event(self, webhook_handler):
        ...
        headers = {"X-GitHub-Event": "unknown_event", "X-GitHub-Delivery": "test-delivery-id"}
    ...
    async def test_process_webhook_error_handling(self, webhook_handler, sample_pr_event):
        ...
        webhook_handler.handle_pull_request = AsyncMock(side_effect=Exception("Processing error"))
    ...
    async def test_queue_for_processing_success(self, webhook_handler):
        ...
        data = {"event_type": "test", "repository": "test/repo", "pull_request_number": 1}
    ...
    async def test_queue_for_processing_failure(self, webhook_handler):
        ...
        webhook_handler.redis_client.rpush = AsyncMock(side_effect=Exception("Redis error"))
...
class TestWebhookEventClass:
    ...
    def test_webhook_event_creation(self):
        ...
        event = WebhookEvent(
            event_type=EventType.PULL_REQUEST, action="opened", payload={"test": "data"}, delivery_id="test-123"
        )
    ...
    def test_webhook_event_to_dict(self):
        ...
        event = WebhookEvent(event_type=EventType.ISSUE_COMMENT, action="created", payload={"comment": "test"})

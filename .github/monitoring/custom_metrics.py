...
class PRProcessingMetrics:
    ...
    def __init__(self, registry: Optional[CollectorRegistry] = None):
        ...
        self.pr_queue_size = Gauge(
            'kiro_pr_queue_size',
            'Current number of PRs in processing queue',
            ['priority'],
            registry=self.registry
        )
        ...
        self.github_rate_limit_remaining = Gauge(
            'kiro_github_rate_limit_remaining',
        ...
        self.slo_pr_processing_time = Summary(
            'kiro_slo_pr_processing_seconds',
            'SLO: PR processing time',
            ['percentile'],
            registry=self.registry
        )
        
        self.slo_availability = Gauge(
            'kiro_slo_availability_ratio',
            'SLO: Service availability ratio',
            registry=self.registry
        )
        
        # System Metrics
        self.active_connections = Gauge(
            'kiro_active_connections',
    ...
    def record_pr_processed(self, status: str, repo_owner: str, repo_name: str, duration: float, pr_size: str):
        ...
        self.pr_processed.labels(
            status=status,
            repo_owner=repo_owner,
            repo_name=repo_name
        ).inc()
        
        self.pr_processing_duration.labels(
            repo_owner=repo_owner,
            repo_name=repo_name,
            pr_size=pr_size
        ).observe(duration)
    ...
    def record_review_posted(self, review_type: str, repo_owner: str, comment_count: int):
        ...
        self.reviews_posted.labels(
            review_type=review_type,
            repo_owner=repo_owner
        ).inc()
        
        self.review_comments.labels(
            repo_owner=repo_owner
    ...
    def get_error_budget(self) -> Dict[str, float]:
        ...
        if self.requests_total > 0:
            ...
            budget['availability'] = {
                ...
                'budget_consumed_percent': (self.requests_failed / allowed_failures * 100) if allowed_failures > 0 else 0
            }

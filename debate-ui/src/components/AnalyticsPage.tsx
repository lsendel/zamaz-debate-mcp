    ...
    <div>
        ...
        <Card title='Recent Activity'>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {debates.slice(0, 5).map(debate => (
              <div
                key={debate.id}
                style={{ paddingBottom: '12px', borderBottom: '1px solid #f0f0f0' }}
              >
            ...
            {debates.length === 0 && (
              <p style={{ fontSize: '14px', color: '#999' }}>No debates yet</p>
            )}
          </div>
        </Card>
      </div>
      ...
      <Card title='Performance Metrics'>
        ...
        >
          ...
          <div>
            <p style={{ fontSize: '14px', color: '#666', marginBottom: '4px' }}>Success Rate</p>
            <p style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>
              {totalDebates > 0 ? `${((completedDebates / totalDebates) * 100).toFixed(1)}%` : '-'}
            </p>
          </div>
        </div>
      </Card>

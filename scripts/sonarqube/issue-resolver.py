...
class IssueResolver:
    ...
    def resolve_issues(self, issues: list[Issue]) -> dict[str, Any]:
        ...
        results = {
            ...
            "skipped": [],
            "errors": [],
            "summary": {"total_issues": len(issues), "fixed_count": 0, "skipped_count": 0, "error_count": 0},
        }
            ...
            try:
                ...
                if fixed:
                    ...
                    results["skipped"].append(
                        {"issue": issue, "reason": f"No automated fix available for rule {issue.rule}"}
                    )
                    results["summary"]["skipped_count"] += 1

            except Exception as e:
                logger.error(f"Error fixing issue {issue.key}: {e}")
                results["errors"].append({"issue": issue, "error": str(e)})
    ...
    def _extract_nested_function(self, lines: list[str], target_line: int) -> str | None:

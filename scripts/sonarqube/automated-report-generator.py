...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('sonarqube_reports.log'),
...
class SonarQubeReportGenerator:
    ...
    def _make_sonar_request(self, endpoint: str, params: Dict[str, Any] = None) -> Dict[str, Any]:
        """Make request to SonarQube API"""
        url = f"{self.sonar_config.url}/api/{endpoint}"
        headers = {
            'Authorization': f'Bearer {self.sonar_config.token}',
    ...
    def collect_project_metrics(self) -> Dict[str, Any]:
        ...
        measures = self._make_sonar_request('measures/component', {
            'metricKeys': ','.join([
        ...
        # Get issues
        issues = self._make_sonar_request('issues/search', {
            'componentKeys': self.sonar_config.project_key,
            'resolved': 'false',
            'ps': 500
        })
        
        # Get security hotspots
        hotspots = self._make_sonar_request('hotspots/search', {
            'projectKey': self.sonar_config.project_key,
            'status': 'TO_REVIEW',
            'ps': 500
        })
        
        # Get quality gate status
        quality_gate = self._make_sonar_request('qualitygates/project_status', {
            'projectKey': self.sonar_config.project_key,
            'branch': self.sonar_config.branch
        })
...
"""
        ...
        for severity in ['BLOCKER', 'CRITICAL', 'MAJOR', 'MINOR', 'INFO']:
            if severity in severity_groups:
                ...
                for issue in severity_groups[severity][:10]:  # Show first 10 issues
                    ...
                    if 'key' in issue:
                        result.append(f"  - [View Issue]({self.sonar_config.url}/project/issues?id={self.sonar_config.project_key}&open={issue['key']})")
                    result.append("")
    ...
    def _generate_hotspots_summary(self, hotspots: List[Dict[str, Any]]) -> str:
        ...
        for category, hotspot_list in category_groups.items():
            ...
            for hotspot in hotspot_list[:5]:  # Show first 5 hotspots per category
                ...
                # Add link to SonarCloud
                if 'key' in hotspot:
                    result.append(f"  - [Review Hotspot]({self.sonar_config.url}/project/security_hotspots?id={self.sonar_config.project_key}&hotspots={hotspot['key']})")
                result.append("")
    ...
    def generate_json_report(self, data: Dict[str, Any]) -> str:
        ...
        json_data = {
            ...
            'metrics': {},
            'issues': {
                'total': len(data.get('issues', {}).get('issues', [])),
                'by_severity': {},
                'by_type': {}
            },
            'security_hotspots': {
                'total': len(data.get('hotspots', {}).get('hotspots', [])),
                'by_category': {}
            },
        ...
        for hotspot in hotspots:
            category = hotspot.get('securityCategory', 'UNKNOWN')
            json_data['security_hotspots']['by_category'][category] = json_data['security_hotspots']['by_category'].get(category, 0) + 1
    ...
    def save_report(self, content: str, format_type: str, timestamp: str = None) -> str:
        """Save report to file"""
        if not timestamp:
    ...
    def send_slack_notification(self, report_data: Dict[str, Any]):
        ...
        try:
                ...
                "attachments": [
                    {
                        ...
                        "fields": [
                            ...
                            {
                                "title": "Bugs",
                                "value": metrics.get('bugs', 'N/A'),
                                "short": True
                            },
                            {
                                "title": "Vulnerabilities",
                                "value": metrics.get('vulnerabilities', 'N/A'),
                                "short": True
                            },
                            {
                                "title": "Code Smells",
                                "value": metrics.get('code_smells', 'N/A'),
                                "short": True
                            },
                            {
                                "title": "Coverage",
                                "value": f"{metrics.get('coverage', 'N/A')}%",
                                "short": True
                            },
                            {
            ...
            response = requests.post(self.notification_config.slack_webhook_url, json=message)
            response.raise_for_status()

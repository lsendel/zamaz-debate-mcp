...
class SonarQubeAnalysisRunner:
    ...
    def _parse_issues_from_data(self, sonar_data: dict) -> list:
        ...
        for hotspot_data in sonar_data.get('hotspots', {}).get('hotspots', []):
            issue = Issue(
                ...
                component=hotspot_data.get('component', '').replace(self.report_generator.sonar_config.project_key + ':', ''),
                ...
                file_path=hotspot_data.get('component', '').replace(self.report_generator.sonar_config.project_key + ':', '')
            )
            issues.append(issue)
    ...
    def _generate_analysis_summary(self, results: dict) -> dict:
        ...
        return {
            ...
            'fix_success_rate': results['issues_fixed'] / results['issues_analyzed'] if results['issues_analyzed'] > 0 else 0,
    ...
    def generate_detailed_report(self) -> str:
        ...
        report += f"- **Trend Analysis**: {'Enabled' if self.report_generator.report_config.include_trends else 'Disabled'}\n"
...
def main():
    ...
    parser.add_argument('--config', default='sonarqube_config.yaml', 

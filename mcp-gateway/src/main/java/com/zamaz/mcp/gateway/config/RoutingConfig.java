    ...
    public RouteLocator customRouteLocator(RouteLocatorBuilder builder) {
        return builder.routes()
            ...
            .route("health_route", r -> r
                ...
                .filters(f -> f
                    .filter(loggingFilter.apply(config -> config.setLogLevel("INFO")))
                    .setResponseHeader("X-Health-Check", LocalDateTime.now().toString())
                )
                .uri("no://op")
            )
            ...
            .route("organization_service", r -> r
                ...
                .filters(f -> f
                    .filter(loggingFilter.apply(config -> config.setLogLevel("DEBUG")))
                    .filter(authFilter.apply(config -> config.setRequireAuth(true)))
                    .filter(validationFilter.apply(config -> config.setValidateOrgId(true)))
            ...
            .route("debate_service", r -> r
                ...
                .filters(f -> f
                    .filter(loggingFilter.apply(config -> config.setLogLevel("DEBUG")))
                    .filter(authFilter.apply(config -> config.setRequireAuth(true)))
                    .filter(validationFilter.apply(config -> config.setValidateDebateAccess(true)))
                    .filter(cachingFilter.apply(config -> config
                        .setCacheName("debates")
                        .setCacheDuration(Duration.ofMinutes(5))
                    ))
                    .requestRateLimiter(config -> config
                        .setRateLimiter(redisRateLimiter)
                        .setKeyResolver(userKeyResolver)
            ...
            .route("llm_service", r -> r
                ...
                .filters(f -> f
                    ...
                    .filter(circuitBreakerFilter.apply(config -> config
                        .setName("llm-cb")
                        .setSlowCallDuration(Duration.ofSeconds(30))
                        .setSlowCallRateThreshold(50)
                    ))
                    .requestRateLimiter(config -> config
                        .setRateLimiter(new RedisRateLimiter(10, 20, 1)) // Lower rate for LLM
                        .setKeyResolver(userKeyResolver)
                    )
                    .modifyRequestBody(String.class, String.class, 
                        (exchange, body) -> {
                            // Add request tracking
                            exchange.getAttributes().put("request.start", System.currentTimeMillis());
                            return body;
                        })
                    .modifyResponseBody(String.class, String.class,
                        (exchange, body) -> {
                            // Add response time tracking
                            Long startTime = exchange.getAttribute("request.start");
                            if (startTime != null) {
            ...
            .route("rag_service", r -> r
                ...
                .filters(f -> f
                    ...
                    .filter(cachingFilter.apply(config -> config
                        ...
                        .setCacheDuration(Duration.ofMinutes(15))
                    ))
                    .requestRateLimiter(config -> config
                        .setRateLimiter(redisRateLimiter)
                        .setKeyResolver(userKeyResolver)
            ...
            .route("template_service", r -> r
                ...
                .filters(f -> f
                    .filter(loggingFilter.apply(config -> config.setLogLevel("DEBUG")))
                    .filter(authFilter.apply(config -> config.setRequireAuth(true)))
                    .filter(validationFilter.apply(config -> config.setValidateTemplateAccess(true)))
                    .filter(cachingFilter.apply(config -> config
            ...
            .route("context_service", r -> r
                ...
                .filters(f -> f
                    .filter(loggingFilter.apply(config -> config.setLogLevel("DEBUG")))
                    .filter(authFilter.apply(config -> config.setRequireAuth(true)))
                    .filter(validationFilter.apply(config -> config.setValidateContextAccess(true)))
                    .requestRateLimiter(config -> config
                        .setRateLimiter(redisRateLimiter)
                        .setKeyResolver(userKeyResolver)
                    )
            ...
            .route("auth_routes", r -> r
                ...
                .filters(f -> f
                    .filter(loggingFilter.apply(config -> config
                        .setLogLevel("INFO")
                        .setLogSensitive(false) // Don't log passwords
                    ))
                    .requestRateLimiter(config -> config
                        .setRateLimiter(new RedisRateLimiter(5, 10, 1)) // Strict rate limit for auth
            ...
            .route("api_docs", r -> r
                ...
                .filters(f -> f
                    ...
                    .filter(cachingFilter.apply(config -> config
                        ...
                        .setCacheDuration(Duration.ofHours(24))
                    ))
                )
                .uri("no://op")
            )
        ...
        return builder.routes()
            ...
            .route("v2_routes", r -> r
                ...
                .filters(f -> f
                    ...
                    .filter(loggingFilter.apply(config -> config.setLogLevel("INFO")))
                    .filter(validationFilter.apply(config -> config.setStrictValidation(true)))
                )
                .uri("no://op")
            )
        ...
        return builder.routes()
            ...
            .route("llm_lb_route", r -> r
                ...
                .filters(f -> f
                    ...
                    .filter(authFilter.apply(config -> config.setRequireAuth(true)))
                    .requestRateLimiter(config -> config
                        .setRateLimiter(new RedisRateLimiter(5, 10, 1))
                        .setKeyResolver(userKeyResolver)
                    )
                    .retry(config -> config
                        .setRetries(2)
    ...
    public RouteLocator canaryRoutes(RouteLocatorBuilder builder) {
        return builder.routes()
            ...
            .route("ab_test_route", r -> r
                ...
                .filters(f -> f
                    .filter(loggingFilter.apply(config -> config.setLogTag("AB-TEST")))

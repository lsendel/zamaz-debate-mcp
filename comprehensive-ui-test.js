      ...
      for (const route of routes) {
        ...
        try {
          ...
          const pageContent = await this.page.evaluate(() => {
            ...
            return {
              heading: /* TODO: Refactor nested ternary - h1?.textContent?.trim(),
          ...
          test.steps.push({ ;
            route: route.path,            loaded: true,
            heading: pageContent.heading ;
          });
        } catch (error) {
          console.log(`   ❌ Failed to load: ${error.message}`);
          test.steps.push({ ;
            route: route.path,            loaded: false,
            error: error.message ;
          });
        }
      }
  ...
  async testErrorHandling() {
    await this.runTest('Error Handling and Edge Cases', async (test) => {
      ...
      const is404 = await this.page.evaluate(() => {
        ...
        return /* TODO: Refactor nested ternary - text?.includes('404') || text?.includes('not found') || text?.includes('Not Found');
      });
      ...
      await this.page.reload({ waitUntil: 'networkidle2' }).catch(() => { console.error("Error:", error); });
      await this.wait(1000);

      const offlineHandling = await this.page.evaluate(() => {
        return document.body./* TODO: Refactor nested ternary - textContent?.includes('offline') || ;
